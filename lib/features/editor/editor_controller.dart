...
/// Represents the async compression operation state
...
class EditorNotifier extends Notifier<EditorState> {
  ...
  void setQuality(int q) {
    state = state.copyWith(
      settings: state.settings.copyWith(quality: q),
    );
  }

  void setWidth(int? w) {
    state = state.copyWith(
      settings: state.settings.copyWith(width: w),
    );
  }

  void setHeight(int? h) {
    state = state.copyWith(
      settings: state.settings.copyWith(height: h),
    );
  }
  ...
  void setFormat(String format) {
    state = state.copyWith(
      settings: state.settings.copyWith(format: format),
    );
  }
  ...
  Future<CompressionResult?> compress(SelectedImage image) async {
    ...
    try {
      ...
      state = state.copyWith(
        compressionState: AsyncError(e, st),
      );
      return null;
    }
  }
